...
sub deflate_encode ($size, $literals, $distances, $lengths, $has_backreference, $out_fh) {
...
sub compress_file ($input, $output) {
    ...
    while (read($fh, (my $chunk), CHUNK_SIZE)) {
        ...
        deflate_encode(length($chunk), \@uncompressed, \@indices, \@lengths, \@has_backreference, $out_fh);
    }
