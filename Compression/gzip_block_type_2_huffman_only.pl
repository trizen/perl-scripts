...
# Create a valid Gzip container, using DEFLATE's Block Type 2 with dynamic prefix codes only, without LZ77.
...
while (read($in_fh, (my $chunk), CHUNK_SIZE)) {
    ...
    my @CL_code;
    foreach my $length (@LL_code_lengths, @distance_code_lengths) {
        push @CL_code, $length;
    }
