...
sub semiprime_equationization {
    ...
    push @result,
      map { ("int x$_;", $_ == $xlen ? "for (x$_ = 1; x$_ <= 9; x$_++) {" : "for (x$_ = 0; x$_ <= 9; x$_++) {") } 0 .. $xlen;
    push @result,
      map { ("int y$_;", $_ == $ylen ? "for (y$_ = 1; y$_ <= 9; y$_++) {" : "for (y$_ = 0; y$_ <= 9; y$_++) {") } 0 .. $ylen;
