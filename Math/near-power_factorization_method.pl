...
use ntheory qw(divisors is_power);
...
if (@ARGV) {
    say join ', ', near_power_factorization(Math::AnyNum->new($ARGV[0]), defined($ARGV[1]) ? $ARGV[1] : ());
    exit;
}

say '2^256  - 1      = ', join ' * ', near_power_factorization(ipow(2,  256) - 1);
say '10^120 + 1      = ', join ' * ', near_power_factorization(ipow(10, 120) + 1);
say '10^120 - 1      = ', join ' * ', near_power_factorization(ipow(10, 120) - 1);
say '10^120 - 25     = ', join ' * ', near_power_factorization(ipow(10, 120) - 25);
say '10^105 - 1      = ', join ' * ', near_power_factorization(ipow(10, 105) - 1);
say '10^105 + 1      = ', join ' * ', near_power_factorization(ipow(10, 105) + 1);
say '10^120 - 2134^2 = ', join ' * ', near_power_factorization(ipow(10, 120) - 2134 * 2134);
